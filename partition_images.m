path1='C:\Users\Asis\Desktop\morphology analysis\test\';
Files=dir(strcat(path1,'*.tiff'));
len=size(Files);
len= len(1);
for i = 1:len
    fullpath=strcat(path1,Files(i).name);
    savepath='C:\Users\Asis\Desktop\morphology analysis\test_segmented\';
    ...
    blockSizeR = round(size(a,1)/2); % Rows in block.
    blockSizeC = round(size(a,2)/2); % Columns in block.
    ...
    for r = 1 : numPlotsR
        for c = 1 : numPlotsC
            ...
            subplot(numPlotsR, numPlotsC, plotIndex);
              ...
              if size(rgbBlock,2)>1 && size(rgbBlock,1)>1
